...
const HistoryCards = ({ Series }: seriesCardProps) => {
  return (
    <Carousel
      opts={{
        align: "start",
        ...
        {Series.map((series, index) => (
          ...
          >
            <div className=" rounded-lg transition-all duration-300 overflow-hidden group">
              ...
              <p className="h-16 font-bold  items-center-safe gap-2 px-2 ">
                <span className="text-xs md:text-sm">
                  {series.title.length > 29
                    ? series.title.slice(0, 29) +
                      `... ( Chapter: ${series.chapter} )`
                    : series.title + ` ( Chapter: ${series.chapter} )`}
                </span>
              </p>
            </div>
          </CarouselItem>
